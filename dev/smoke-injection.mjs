...
await WorldStateMachine.Engine.setEnabled(false);
await WorldStateMachine.Engine.interceptor(outboundChat, 8192, () => {}, 'normal');
assert.equal(registeredPromptArgs[1], '');
assert.equal(WorldStateMachine.Injection.compose(state), '', '总开关关闭后不得合成任何正文注入');
assert.deepEqual(WorldStateMachine.Injection.composeByDepth(state), {}, '总开关关闭后不得留下任一深度注入');
WorldStateMachine.Settings.update({ enabled: true });
await WorldStateMachine.Engine.setEnabled(true);
...
assert.equal(WorldStateMachine.Storage.load().runtime.lastReadFloor, legacyRollbackTarget.runtime.lastReadFloor, '回滚必须恢复快照的读取位置，不能保留被删除楼层的高水位');
