...
const olderCompression=structuredClone(state);
olderCompression.runtime.worldbookRead[entry.key]=`logic-v2:${W.Facts.hash(entry.content)}`;
assert.equal(W.WorldbookSemantic.hasRead(olderCompression,entry),false,'old summary receipt does not certify the new fidelity rule');
...
assert.ok(html.includes('已按精简规则读取'));
assert.ok(html.includes('普通正文结算只读取精简记忆和上一轮正文'));
assert.ok(html.includes('不代表逐项信息已经自动验全'));
