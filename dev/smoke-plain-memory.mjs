...
let forbidSourceRead = false;
WSM.Context.buildSource = async options => { sourceReads.push(options); assert.equal(forbidSourceRead,false,'ordinary body reads must never acquire original sources'); return {...structuredClone(realSource),worldbooks:originalBooksEnabled ? structuredClone(realSource.worldbooks) : []}; };
...
const beforeBodySourceReads = sourceReads.length;
forbidSourceRead = true;
result = await WSM.Engine.settle({force:true,latestOnly:true});
forbidSourceRead = false;
assert.ok(result);
assert.equal(requests.length,3,'subsequent reconciliation, fill and simulation share one call');
assert.equal(requests[2].input.source,undefined,'normal update excludes originals even while their books stay enabled');
assert.equal(sourceReads.length,beforeBodySourceReads,'even missing NPC activity cannot trigger source acquisition');
...
assert.deepEqual(WSM.Storage.load().memory.characters,['张三已搬到杭州']);
assert.equal(requests.length,6,'body truncation allows exactly two continuation attempts');
mode='normal';
...
assert.equal(requests.length-beforeFailureCalls,4,'transient final-phase errors have two bounded continuation attempts');
...
assert.equal(WSM.PlainMemory.canInitialize(WSM.Storage.load()),true,'unfinished initialization can resume without clearing memory');
mode='normal';
const resumeCalls=requests.length;
await WSM.Engine.plan({initialize:true});
assert.equal(requests.length-resumeCalls,1,'manual resumption goes directly to the unfinished reasoning phase');
...
assert.match(result.error,/自动接续已停止.*继续初始化/);
...
const repairSourceReads=sourceReads.length;
const unreadReceipts=structuredClone(WSM.Storage.load().runtime.worldbookRead);
forbidSourceRead=true;
...
assert.equal(sourceReads.length,repairSourceReads,'unfinished initialization cannot expand an ordinary body read to full history');
assert.deepEqual(WSM.Storage.load().runtime.worldbookRead,unreadReceipts,'body settlement must not mark original sources read');
assert.equal(WSM.Storage.load().runtime.plainInitIncomplete,true,'only explicit initialization may certify original-source coverage');
assert.equal(WSM.Storage.load().runtime.plainReadIncomplete,false);
assert.equal(WSM.PlainMemory.canInitialize(WSM.Storage.load()),true,'the unfinished initialization can still be resumed explicitly');
await WSM.Storage.clearAll();
