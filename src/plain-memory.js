...
旧memory不天然正确：发现强化标签、主观根因、抽象的加码路径或尚未发生的关系终局，按source复核后用before逐字替换或删除错误部分，保留已确认事实和实际约束；不能因旧条未变就KEEP错误解释。无新事件、条件或明确表达时不增加行为强度，不因多读一轮就加重状态，允许人物缓和、协商、停滞或改变。不得替玩家决定感受、顺从、依赖、失去自主性或任何关系结局。`;
    ...
    const CARD_MEMORY = `常驻角色卡简记规则：char的角色卡source.character和user的Persona/source.persona持续提供给正文AI，不会因记忆读取而关闭；完整阅读以理解人物，但不把整张卡复制进记忆。char与user在characters中各用一句简要概况，保留身份、当前位置、必要处境，以及驱动自主活动的简要性格、能力、意愿与目标；没有变化就保留现有简述。角色卡标题不一定是人名，多角色卡按实际人物区分。
...
人物概况保留简要身份、性格、能力、意愿、位置与目标。正文已写的地点必须落到对应人物概况和活动记录，用before替换旧句并保留有效部分。无移动证据沿用已知位置；推演阶段才依据已有记忆补缺并标明“推测”，不把未来目的地、回忆或远程事务地点当作本人位置。地图保留层级与通行条件，组织与个人分栏。
