...
assert.equal(takeoverSource.worldbooks[0].entries.length,0,'merged reads must honor saved entry selection');
...
assert.equal(compiled.count, 1);
assert.equal(WorldStateMachine.WorldbookCompiler.getReport().entries.some((entry) => entry.entryId === '2'), false, '禁用条目不发送给编译 API');
...
assert.equal(compilePayloads.length, callsAfterInitialCompile, '修改禁用条目不能触发 API 读取');
